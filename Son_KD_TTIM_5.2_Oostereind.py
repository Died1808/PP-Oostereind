...
##############################################K-waarde        
    ...
    def plt(self, filename):
        ...
        df = pd.concat([df, pd.DataFrame.from_records([{'depth':-37.8,'k': 0.8}])], ignore_index=True)
        df = pd.concat([df, pd.DataFrame.from_records([{'depth':-50,  'k': 35}])], ignore_index=True)
        df = pd.concat([df, pd.DataFrame.from_records([{'depth':-54,  'k': 0.04}])], ignore_index=True)
        df = pd.concat([df, pd.DataFrame.from_records([{'depth':-140, 'k': 18}])], ignore_index=True)
        df = pd.concat([df, pd.DataFrame.from_records([{'depth':-141, 'k': 1e-4}])], ignore_index=True)
        ...
        df['S'] = np.where(df['depth']> -2, 7.5e-3, 1e-4)
        df['S'] = np.where(df['depth']< -10, 2e-5, df['S']) 
        df['S'] = np.where(df['depth']< -38, 5e-6, df['S']) 

        df['Cc'] = (df['k']/(24*60*60)*df['kzkh']/df['S'])/2.5 #

        ml=Model3D(kaq=df['k'], z=dfn[ 'depth'], Saq= df['S'], kzoverkh=df['kzkh'], tmin=1e-4, 
                   tmax=25, M=10)
        ...
        for rnum in well.index:
            ...
            x    = well['x'][rnum] 
            y    = well['y'][rnum]
            punt = Well(ml, x,y, tsandQ=[(0, 24*50.5),(20,0)], 
                        rw=0.25, layers=well1)

        laag1 = df['depth'].sub(float(-8)).abs().values.argmin() 
        laag2 = df['depth'].sub(float(-22)).abs().values.argmin() 
        laag3 = df['depth'].sub(float(-83)).abs().values.argmin() 
...
####################################
        ml.solve()
# ###################################
        onderlaag = abs(dzend)-abs(OZaq)
        
        KD= round((df['k'].sum()/2.5+onderlaag*df.iloc[-2,1]),0)
        print('KD  =', KD, ' [m2/dag]')
        
        a=np.amin(punt.headinside(20))
        print('HeadInside = ',round(a,2), ' [m NAP]')

        for la in laag:
            ml.xsection(x1=x1, x2=x1, y1=y1, y2=y1+1000, npoints=2500, t=20,
        ...
        plt.yticks(np.arange(0.25,-3.0,-0.25))
        plt.title('Verlaging t.g.v pompproef Oostereind  KD = '+ str(int(round(KD,0)))+' [m2/dag]')
